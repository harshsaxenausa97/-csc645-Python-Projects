...
Network socket is a module used to communicate between two computers. It provides two types of interface to access the network, namely low-level (platform dependent connections — Example: Telnet) and high-level (application dependent connections — Example: HTTP, FTP, SMTP, etc.). This project is to establish the low-level socket connection between server and client to communicate messages using the TCP/IP protocol. The chat application I made more like a chat room. So, this means that multiple users can connect to the chat server and send their messages. Every message is broadcasted to every connected chat user.

The code consists of 4 python scripts. First is the server.py and the other is the chat client.py and supported class of client_handler.py and menu.py.
...
In this project, I have used server.py to serve data by sending an invitation to the client. After accepting the invitation, both server and client share messages mutually. The chat server does the following things
...
A server script performs the sequence of functions such as socket(), bind(), listen(), and accept_client(), send(), receive(), client_handler_thread() (repeats for more than one client) to communicate with the client. The description of each functions used in the server script are given bellow:
...
* accept_client() – waits and accepts connection request from the client(s)
* receive() – receives message sent through TCP
* Send() – Serialize the data with pickle and send using the accepted client socket

## Chat Client
The chat client that connect to the above chat server. The client is based on the telnet program in python. It connects to a remote server, sends messages and receives messages. client.py to receive data on acceptance of the invitation.
...
![GitHub Server](images/1a.png)

Note down the local IP address & Port and pass it to the clients (invitation). 

2.	Run the client.py script in Python application using the local IP address sent by the server (accept the invitation). (Open 3 Terminals)
```
$ cd client
$ python3 client.py
```

![GitHub Client1](images/1a-a.png) ![GitHub Client2](images/1a-b.png) ![GitHub Client3](images/1a-c.png)

3.	Using various menu options, a client can get client list, send message to another client, get own messages. Can create a new chat channel, join a chat channel, and communicate to other clients. After disconnecting from chat channel, a client can get the menu by typing “Menu” option. 

![GitHub Client1a](images/2b-a.png) ![GitHub Client2a](images/2b-b.png) ![GitHub Client3a](images/2b-c.png)

## Challenges/Limitation: 
When client communicate in chat room, the incoming messages from others client in chat room will be displayed immediately on the console. There is a possibility that client message will be displayed immediately as client is waiting for user to type the message. This happens because incoming message thread and send message function are using same I/O. I did not use lock, because I wanted to display incoming messages immediately without waiting for the client interaction.
When chat room is closed by the owner, the member clients has to explicitly type 'MENU' in order to exit the chat room. In this scenario, I am not able to kill outgoing message function as it is waiting for user interaction from console and blocking the I/O. I am not able to successfully un-block the I/O expecting an input from console. In this case, user has to type ‘MENU’ explicitly to exit chat room.
## --------------------------------------------------------------------------------------------
